...
###############################################################################
## To Run: python3 convert_html2xml_for_pipeline.py -i <input_directory> -o <output_xml_file>
###############################################################################
...
def convert_html2xml(input_file, article_counter):
    ...
    with open(input_file) as html:
        ...
        h1 = post_title.text
        if h1:
            child1 = etree.SubElement(article_element, "div", attrib={"class": "title"})
            child1.text = h1
        ...
        if date:
            child2 = etree.SubElement(article_element, "div", attrib={"class": "date"})
            child2.text = date

        # get author
        author = post_time[0].text
        if author:
            child3 = etree.SubElement(article_element, "div", attrib={"class": "author"})
            child3.text = author
        ...
        for element in content:
            if element.name == "h5":
                ...
                if len(element.text) > 1:
                    par = etree.SubElement(article_element, "div")
